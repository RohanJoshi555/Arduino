...
const float timeLapseInterval = 0.1; //seconds between successive shots
...
void setup() {
  ...
  if(psramFound()){
    config.frame_size = FRAMESIZE_UXGA; //UXGA=1600x1200 FRAMESIZE_ + QVGA|CIF|VGA|SVGA|XGA|SXGA|UXGA
  ...
  // Uses PWM channel 7 to control the white on-board LED flashlight connected to GPIO 4
  ledcSetup(7, 5000, 8);
  ledcAttachPin(4,7);
...
void loop() {
  ...
  delay(timeLapseInterval*1000);
  
  //Set the Brightness of the LED Flash
